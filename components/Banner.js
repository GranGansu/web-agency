...
export default function Banner() {
    ...
    return <div className={`text-4xl sm:text-6xl p-6 my-6 h-[50vh] flex items-center bg-center bg-cover relative border-red-400  text-black overflow-hidden border-y`}>
        <div className={margen + ' font-bold uppercase text-center z-10'}>
            <p>Diseñamos <span className="text-red-400 text-6xl">webs</span></p>
            <motion.div className="mt-4 text-6xl space-y-2 z-10" variants={container} initial="hidden" animate="show">
                ...
                <motion.p className="flex flex-row items-center" variants={item}><Image className="mr-2 opacity-70" alt="" src="/img/circulo2.png" width="30" height="30"></Image> -1Mb</motion.p>
            </motion.div>
        </div>
        <motion.div transition={{ delay: 1, duration: 2 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`left-0 absolute flex flex-col w-[2000px] overflow-hidden h-full z-0 text-gray-100 ${barlow.className}`}>
